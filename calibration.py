...
class WifiManager:
    ...
    def __init__(self, password: str, interface: str = "", connect_timeout: int = 45):
        ...
        self.interface = interface or ("wlan0" if IS_LINUX else "")
    ...
    def disconnect(self, ssid: str = ""):
        if IS_WINDOWS:
            ...
            self._run_nmcli("connection", "down", "id", self.interface)
    ...
    def _connect_linux(self, ssid: str) -> bool:
        ...
        while time.time() - start < self.connect_timeout:
            ...
            time.sleep(5)
            stdout, _, _ = self._run_nmcli("-t", "-f", "NAME,DEVICE", "connection", "show", "--active")
            if ssid in stdout:
                logger.info("Wi-Fi: подключено к %s", ssid)
                return True
            if attempt % 3 == 0:
...
async def phase1_discover_cameras(config: dict, db_session_factory) -> list:
    """Сканирует BLE, находит GCBT40, записывает MacAddress в PhotoTrap."""
    logger.info("=" * 50)
...
async def phase2_bind_ssids(config: dict, cameras: list, db_session_factory) -> list:
    """Для каждой камеры без SSID — находит и привязывает Wi-Fi сеть."""
    logger.info("\n" + "=" * 50)
    ...
    interface = "wlan0" if IS_LINUX else ""

    wifi = WifiManager(wifi_password, interface, connect_timeout)
...
async def run_calibration(config_path: str = "appsettings.json"):
    ...
    # Загружаем конфигурацию: БД > файл > defaults
    async with db_session_factory() as session:
